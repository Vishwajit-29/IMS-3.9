...
class DataService {
  ...
  async getItems() {
    try {
      ...
      const endpoints = ["/items", "/api/items", "items"];
      ...
      if (!response) {
        try {
          ...
          response = await axios.get(`${this.apiBaseUrl}/items`);
      ...
      return response.data.map((item) => {
        ...
        return {
          ...
          imageUrl:
            item.imageUrl || this.getDefaultImageForCategory(item.category),
        };
      });
    } catch (error) {
  ...
  async getCategories() {
    try {
      ...
      // Try multiple endpoints in sequence
      const endpoints = ["/categories", "/api/categories", "categories"];
      ...
      if (!response) {
        try {
          console.log("Trying direct endpoint call...");
          response = await axios.get(`${this.apiBaseUrl}/categories`);
          console.log("Direct endpoint successful:", response.data);
        } catch (directErr) {
          console.error("Direct call also failed:", directErr);
          throw lastError || directErr;
  ...
  async addItem(itemData) {
    try {
      ...
      if (itemData._id) {
        console.log(
          "Found item ID, updating existing item instead of adding new one:",
          itemData._id,
        );
      ...
      const formattedItem = {
        ...
        imageUrl:
          itemData.imageUrl ||
          itemData.image ||
          this.getDefaultImageForCategory(itemData.category),
      ...
      const response = await this.tryMultipleEndpoints(
        (endpoint) => this.api.post(endpoint, formattedItem),
  ...
  async updateItem(itemId, itemData) {
    try {
      ...
      const endpoints = [`/items/${itemId}`, `/api/items/${itemId}`];
      ...
      for (const endpoint of endpoints) {
        try {
          ...
          if (response.data) {
            console.log(
              "Item updated successfully with endpoint:",
              endpoint,
              response.data,
            );
            break;
  ...
  async updateItemQuantity(itemId, quantityChange) {
    try {
      ...
      if (error.response) {
        ...
        console.error("Response data:", error.response.data);
        throw new Error(
          `Failed to update quantity: ${error.response.data.error || error.message}`,
        );
      }
  ...
  async removeItem(itemId, quantity = null) {
    try {
      console.log(
        "Removing item from MongoDB:",
        itemId,
        quantity ? `(Quantity: ${quantity})` : "(Complete removal)",
      );
      ...
      try {
        if (quantity !== null) {
          ...
          const directResponse = await axios.patch(
            `${this.apiBaseUrl}/items/${itemId}/quantity`,
            {
              quantity: -quantity,
            },
          );
          console.log(
            "Item quantity updated with direct API call:",
            directResponse.data,
          );
          return directResponse.data;
        } else {
          ...
          console.log(
            "Item removed with direct API call:",
            directResponse.data,
          );
          return directResponse.data;
        }
  ...
  async getSalesData() {
    try {
      ...
      // Try multiple endpoints in sequence
      const endpoints = ["/sales", "/api/sales", "sales"];
      ...
      if (!response) {
        try {
          console.log("Trying direct endpoint call...");
          response = await axios.get(`${this.apiBaseUrl}/sales`);
          console.log("Direct endpoint successful:", response.data);
        } catch (directErr) {
          console.error(
            "Direct call also failed, generating mock data instead:",
            directErr,
          );
  ...
  async generateMockSalesData() {
    ...
    for (let i = 6; i >= 0; i--) {
      ...
      const stockAdded =
        date.getDay() === 5
          ? Math.round(sales * 1.5)
          : date.getDay() === 1
            ? Math.round(sales * 0.8)
    ...
    for (let i = 11; i >= 0; i--) {
      ...
      if (monthIndex === 11)
        seasonalFactor = 1.6; // December (holiday season)
      else if (monthIndex === 0)
        seasonalFactor = 0.7; // January (post-holiday slump)
      else if (monthIndex >= 5 && monthIndex <= 7)
        seasonalFactor = 1.2; // Summer months
  ...
  async generateTopSellingMockData() {
    try {
      ...
      console.error(
        "Error generating top selling items from inventory:",
        error,
      );
  ...
  async sellItem(itemId, quantityToSell) {
    try {
      ...
      const endpoints = [`/items/${itemId}/sell`, `/api/items/${itemId}/sell`];
      ...
      try {
        ...
        const directResponse = await axios.post(
          `${this.apiBaseUrl}/items/${itemId}/sell`,
          {
      ...
      if (error.response) {
        ...
        console.error("Response data:", error.response.data);
        throw new Error(
          `Failed to sell item: ${error.response.data.error || error.message}`,
        );
      }
    ...
    try {
      ...
      if (!response) {
        ...
        response = await axios.get(
          `${this.apiBaseUrl}/sales/history/${itemId}`,
        );
        console.log("Direct endpoint successful:", response.data);
      }
  ...
  async updateAllItemPrices() {
    try {
      ...
      const endpoints = ["/items/update-prices", "/api/items/update-prices"];
      ...
      for (const endpoint of endpoints) {
        try {
          ...
          lastError = err;
          console.error(
            `Failed to update prices with endpoint ${endpoint}:`,
            err,
          );
        }
      }
      ...
      if (!response) {
        ...
        response = await axios.patch(`${this.apiBaseUrl}/items/update-prices`);
        console.log("Direct endpoint successful:", response.data);
      }
  ...
  async getRecentTransactions() {
    try {
      ...
      try {
        ...
        const items = await this.getItems();
        const itemsWithSales = items.filter(
          (item) => item.sales && item.sales > 0,
        );

        let allTransactions = [];

        // For each item with sales, get its sales history
        for (const item of itemsWithSales.slice(0, 3)) {
          // Limit to 3 items to avoid too many requests
          try {
            const itemHistory = await this.getItemSalesHistory(
              item._id || item.id,
            );
            ...
            console.error(
              `Failed to get sales history for item ${item.name}:`,
              err,
            );
          }
        }
        ...
        if (allTransactions.length > 0) {
          console.log(
            "Retrieved transactions from item sales history:",
        ...
        console.error(
          "Failed to retrieve transactions from item sales history:",
          err,
        );
      }
