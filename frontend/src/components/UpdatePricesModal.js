...
const UpdatePricesModal = ({ onClose, items: allItems }) => {
  ...
  const handleUpdatePrice = async () => {
    ...
    try {
      ...
      // Use dataService to update item (uses correct API URL)
  ...
  const renderItemSelection = () => {
    return (
      <div className="modal-body">
        ...
        >
          <h3 style={{ margin: 0 }}> Select an Item to Update Price </h3>{" "}
        </div>
        ...
        <div className="items-grid">
          ...
          {filteredItems.length === 0 ? (
            <div className="empty-state">
              <div className="empty-icon"> 📦 </div> <h3> No items found </h3>{" "}
      ...
      <div className="modal-content">
        <div className="modal-header">
          <h2 className="modal-title">
            ...
            {step === 2 &&
              `Update Prices - ${selectedCategory === "all" ? "All Categories" : selectedCategory}`}{" "}
            {step === 3 &&
              "Update Price - " + (selectedItem ? selectedItem.name : "")}{" "}
          </h2>{" "}
